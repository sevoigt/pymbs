...
/*****************************************************************************/
{
    ...
    //TODO: kompiliervorgang �berpr�fen, evtl. existenz vom gcc pr�fen, und evtl. auch alternativ den VS Compiler nutzen
	...
	// TODO: Ordner wieder l�schen - oder auch nicht... oder konfigurierbar?

	//Wahrscheinlich sollten alle Dateien/Verzeichnisse einzeln mit remove gel�scht werden. remove_all ist schon relativ b�se...
	//Alternativ das Verzeichnis im Temp ordner anlegen und wieder l�schen.
...
//	Graph::VariableVec sensors = a->getVariables(SENSOR); //Da die Ausgabe gleich ist kann zusammengefasst werden:
	...
	xml::writer xw(f);
    {
			...
			for (size_t i = 0; i < m; ++i)
				for (size_t j = 0; j < n; ++j)
				{
					std::string name = (*it)->getName() + (m>1 ? "_" + str(i) : "") + (n>1 ? "_" + str(j) : "");
					{
						...
						xml::element sv("ScalarVariable", xw);
						sv.attr("name", "der_" + name); // TODO: evtl k�nnte man hier auch die Klammervariante wie beim comment nehmen
						sv.attr("valueReference", m_valueReferences.size());
		...
		for (Graph::VariableVec::iterator it=parameter.begin();it!=parameter.end();++it)
		{
			size_t m = (*it)->getShape().getDimension(1);
			size_t n = (*it)->getShape().getDimension(2);
			for (size_t i = 0; i < m; ++i) // Parameter sind wohl immer Skalar, da eine Skalarisierung schon im Python Teil durchgef�hrt wird??
				for (size_t j = 0; j < n; ++j)
				{
					std::string name = (*it)->getName() + (m>1 ? "_" + str(i) : "") + (n>1 ? "_" + str(j) : "");
					xml::element sv("ScalarVariable", xw);
